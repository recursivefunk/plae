...
img=images;loadImages(config);if(!config.xml)
config.xml='xml/songs.xml';BrowserDetect.init();$.ajax({type:"GET",url:config.xml,dataType:"text/xml",success:parseXml,error:function(xml){}});var initButtons=function(){i=img;var play_=document.getElementById('play');var skip_=document.getElementById('skip');var stop_=document.getElementById('stop');var back_=document.getElementById('back');$('#play-link').click(function(){play(curr_song);return false;});$('#play-link').mouseover(function(){play_.setAttribute('src',i[1].src);});$('#play-link').mouseout(function(){play_.setAttribute('src',i[0].src);});$('#skip-link').click(function(){skip();return false;});$('#skip-link').mouseover(function(){skip_.setAttribute('src',i[5].src);});$('#skip-link').mouseout(function(){skip_.setAttribute('src',i[4].src);});$('#stop-link').click(function(){stopMusic(curr_song.toString());return false;});$('#stop-link').mouseover(function(){stop_.setAttribute('src',i[7].src);});$('#stop-link').mouseout(function(){stop_.setAttribute('src',i[6].src);});$('#back-link').click(function(){skipBack();return false;});$('#back-link').mouseover(function(){back_.setAttribute('src',i[9].src);});$('#back-link').mouseout(function(){back_.setAttribute('src',i[8].src);});}
...
initButtons();showSongInfo();if(config.useArt===true){document.getElementById('art').setAttribute('src',songs[curr_song].thumb.src);}}}
...
function buttonPauseState(){var i=img;var play=document.getElementById('play');var play_link=document.getElementById('play-link');play.setAttribute('src',i[2].src);$('#play-link').mouseout(function(){play.setAttribute('src',i[2].src);});$('#play-link').mouseover(function(){play.setAttribute('src',i[3].src);});}
function buttonPlayState(){var i=img;var play=document.getElementById('play');play.setAttribute('src',i[0].src);$('#play-link').mouseout(function(){play.setAttribute('src',i[0].src);});$('#play-link').mouseover(function(){play.setAttribute('src',i[1].src);});}
function showSongInfo(){var song_=songs[curr_song];document.getElementById('info').innerHTML="<p>"+song_.artist+"  </br>"+song_.title+" </p>";}
...
{console.log('def not IE 9');local_song.id=$(this).attr("id");local_song.title=$(this).attr("track");local_song.url=$(this).attr("url");local_song.artist=$(this).attr("artist");local_song.thumb=new Image();local_song.thumb.src=$(this).attr("thumb");songs[i]=jQuery.extend(true,{},local_song);i=i+1;});}
else{$(xml).find("song").each(function()
{console.log('might b ie9');local_song.id=$(this).attr("id");local_song.title=$(this).attr("track");local_song.url=$(this).attr("url");local_song.artist=$(this).attr("artist");local_song.thumb=new Image();local_song.thumb.src=$(this).attr("thumb");local_song.duration=$(this).attr("duration");songs[i]=jQuery.extend(true,{},local_song);i=i+1;});}}
