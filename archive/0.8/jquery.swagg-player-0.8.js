		...
		function swaggOn(swagg_div, config) {
				...
				var initButtons = function() {
					...
					$('#play-link').mouseover(function() {
						 play_.setAttribute('src', img[1].src);
					});
					$('#play-link').mouseout(function() {
						 play_.setAttribute('src', img[0].src);
					});
					...
					$('#skip-link').mouseover(function() {
						 skip_.setAttribute('src', img[5].src);
					});
					$('#skip-link').mouseout(function() {
						 skip_.setAttribute('src', img[4].src);
					});
					...
					$('#stop-link').mouseover(function() {
						 stop_.setAttribute('src', img[7].src);
					});
					$('#stop-link').mouseout(function() {
						 stop_.setAttribute('src', img[6].src);
					});
					...
					$('#back-link').mouseover(function() {
						 back_.setAttribute('src', img[9].src);
					});
					$('#back-link').mouseout(function() {
						 back_.setAttribute('src', img[8].src);
					});
				}
				...
				// to be fired in the event that the soundManager onload function
				// completes BEFORE the first element in the song array is defined
				// will confinue to fire every 10 milliseconds until the first element 
				// is defined
				...
				soundManager.onload =  function() {
					interval_id = setInterval('soundManager.createSongs()', 5);	
			...
			function buttonPauseState() {
				...
				play.setAttribute('src', img[2].src);
				
				$('#play-link').mouseout(function() {
					play.setAttribute('src', img[2].src);
				});
				$('#play-link').mouseover(function() {
					play.setAttribute('src', img[3].src);
				});
			}
			...
			function buttonPlayState() {
				var play = document.getElementById('play');
				play.setAttribute('src', img[0].src);
				$('#play-link').mouseout(function() {
					play.setAttribute('src', img[0].src);
				});
				$('#play-link').mouseover(function() {
					play.setAttribute('src', img[1].src);
				});
			}
			
			// displays artist [song title]
			function showSongInfo(){
				document.getElementById('info').innerHTML = "<p>" + songs[curr_song].artist + "  </br>" + songs[curr_song].title + " </p>";	
			}
